...
function renderCraftInfo(craft_id) {
	...
	let object_str = craft_info?.monster ? craft_info.monster.map(m => `<img class='skill_tooltip monster_img' src='../tos_tool_data/img/monster/${m}.png' onerror='this.src=&quot;../tos_tool_data/img/monster/noname${monster_data.find(monster => monster.id === m)?.attribute ? `_${attr_zh_to_en[monster_data.find(monster => monster.id === m)?.attribute]}` : ''}.png&quot;' title='${`No.${m} ${monster_data.find(monster => monster.id === m)?.name}`}' />`).join('') : craft_info?.series ? `擁有${craft_info?.series.map(s => `<span class='craft_object_series'>【${s}】</span>`).join('、')}特性` : (
